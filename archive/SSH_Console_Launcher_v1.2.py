"""
Embedded SSH Console Launcher for Windows - Version 1.2
...
APP_NAME = "Embedded SSH Launcher v1.2"
...
class EmbeddedSSHLauncher(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.profile_list = tk.Listbox(sidebar, height=7, width=34)
        ...
        ttk.Button(
            layout,
            text="Clear Console",
            command=self.clear_focused_console,
        ).pack(fill="x", pady=2)
        ...
        commands_frame = ttk.LabelFrame(sidebar, text="Quick Commands", padding=8)
        commands_frame.pack(fill="both", expand=True, pady=(0, 10))

        self.command_list = tk.Listbox(commands_frame, height=8, width=34)
        self.command_list.pack(fill="both", expand=True, pady=(0, 6))
        self.command_list.bind("<<ListboxSelect>>", self.on_select_command)
        self.command_list.bind("<Double-Button-1>", lambda _event: self.run_selected_command())
        ...
        ttk.Label(
            topbar,
            text="Double-click a profile to open SSH. Double-click a command to run it in the focused console. Click × on a tab to close it.",
        ).pack(side="left")
    ...
    def refresh_commands(self) -> None:
        ...
        for command in self.commands:
            self.command_list.insert("end", command.name)
    ...
    def on_select_command(self, _event: object | None = None) -> None:
    ...
    def selected_command(self) -> QuickCommand | None:
        ...
        if self.selected_command_index is None:
            messagebox.showerror(APP_NAME, "Select a command first.")
            return None
    ...
    def edit_command(self) -> None:
        ...
        CommandStore.save(self.commands)
        self.refresh_commands()
        self.command_list.selection_set(old_index)
        self.selected_command_index = old_index
    ...
    def run_selected_command(self) -> None:
        ...
        self.run_command_on_focused_console(selected.command)
        self.status_var.set(f"Ran command: {selected.name}")
