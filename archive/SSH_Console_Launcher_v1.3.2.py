"""
Embedded SSH Console Launcher for Windows - Version 1.3 UI Refresh
...
APP_NAME = "Embedded SSH Launcher v1.3"
...
class EmbeddedSSHLauncher(ctk.CTk if ctk is not None else tk.Tk):
    ...
    def on_notebook_button_release(self, event: tk.Event) -> None:
        if self.pending_tab_close_id is None:
            return
        ...
        # If the mouse moved too much, treat it as a drag/select/focus action, not close.
        ...
        if release_candidate == self.pending_tab_close_id:
            tab_id = self.pending_tab_close_id
            self.pending_tab_close_id = None
            self.pending_tab_close_press_xy = None
